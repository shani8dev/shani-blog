 ...
 * To add a new brand, copy config-shani.js, rename it, and fill
...
// ── Storage key helper ─────────────────────────────────────────────
const _pfx = (CONFIG.STORAGE_PREFIX || 'blog') + '_';
const _key = k => _pfx + k;

// TAG_ICONS and Utils are defined in utils.js — loaded before this file.
...
const KeysDB = {
  ...
  async fetch() {
    ...
    const url = CONFIG.MEMBERSHIP_KEYS_URL;
    if (!url) return [];
    try {
...
// =========================================
// 2. UTILS — defined in utils.js (loaded before this file)
// =========================================
...
const DataLoader = {
  ...
  async load() {
    ...
    } catch { /* manifest not present yet — fall through */ }
...
const Renderer = {
  ...
  applyBranding() {
    ...
    // ── Inject membership copy if present on homepage ─────────────────
    const memHeading = Utils.qs('[data-config="MEMBERSHIP_HEADING"]');
    if (memHeading) memHeading.textContent = CONFIG.MEMBERSHIP_HEADING;
    const memDesc = Utils.qs('[data-config="MEMBERSHIP_DESCRIPTION"]');
    if (memDesc) memDesc.textContent = CONFIG.MEMBERSHIP_DESCRIPTION;
    const memCta = Utils.qs('[data-config="MEMBERSHIP_CTA"]');
    if (memCta) memCta.textContent = `${CONFIG.MEMBERSHIP_CTA} — ${CONFIG.MEMBERSHIP_PRICE}`;
  },
  ...
  renderPost(post) {
    ...
    if (paywalled && !AppState.isMember) {
      ...
      <div class="paywall-gate"><div class="paywall-card">
        ...
        <div class="paywall-actions">
          <button class="btn primary" id="paywall-membership-btn">
            <i class="fa-solid fa-star"></i> Become a Member — ${Utils.escapeHtml(CONFIG.MEMBERSHIP_PRICE || '₹199 / year')}
          </button>
    ...
    app.querySelector('#paywall-membership-btn')?.addEventListener('click', () => {
      history.pushState({}, '', '/membership');
      Router.render();
    });
    ...
    if (unlockBtn && licenseInput) {
      ...
      const doVerify = async () => {
        ...
        try {
          const data = await KeysDB.validate(key);
          ...
          setStatus('Could not load key database. Check your connection and try again.', true);
          unlockBtn.disabled = false;
          unlockBtn.innerHTML = '<i class="fa-solid fa-unlock"></i> Unlock';
        }
      };
        ...
        if (insertAfter) {
          ...
          if (CONFIG.ADSENSE_CLIENT && CONFIG.ADSENSE_SLOT) {
            ...
            if (sessionStorage.getItem(_key('house_ad_dismissed')) !== '1') {
              adWrap.innerHTML = `
                <div class="house-ad">
                  ...
                  <div class="house-ad__footer">
                    <button class="house-ad__cta btn primary" id="house-ad-cta-btn">
              ...
              adWrap.querySelector('#house-ad-cta-btn')?.addEventListener('click', () => {
                history.pushState({}, '', '/membership');
                Router.render();
              });
            }
...
const Router = {
  ...
  async render() {
    ...
    const memberEl  = document.getElementById('view-membership');
    const readBar   = document.getElementById('reading-bar');

    // ── Helper: hide all non-active top-level views ───────────────
    const hideOtherViews = (keep) => {
      if (indexEl   && keep !== 'index')      indexEl.style.display  = 'none';
      if (postEl    && keep !== 'post')       postEl.style.display   = 'none';
      if (memberEl  && keep !== 'membership') memberEl.style.display = 'none';
      const bmView = document.getElementById('view-bookmarks');
      if (bmView    && keep !== 'bookmarks')  bmView.style.display   = 'none';
      if (readBar   && keep !== 'post') { readBar.style.display = 'none'; readBar.style.width = '0%'; }
    };

    // ── Membership view (/membership) ────────────────────────────
    if (location.pathname === '/membership') {
      hideOtherViews('membership');
      if (memberEl) memberEl.style.display = '';
      document.title = `${CONFIG.MEMBERSHIP_HEADING} — ${CONFIG.SITE_TITLE}`;

      document.getElementById('canonical-url')?.setAttribute('href', `${CONFIG.BLOG_URL}/membership`);
      document.getElementById('og-url')?.setAttribute('content', `${CONFIG.BLOG_URL}/membership`);
      document.getElementById('og-type')?.setAttribute('content', 'website');
      ['og-title', 'tw-title'].forEach(id => Utils.qs(`#${id}`)?.setAttribute('content', `${CONFIG.MEMBERSHIP_HEADING} — ${CONFIG.SITE_TITLE}`));
      ['og-desc', 'tw-desc'].forEach(id => Utils.qs(`#${id}`)?.setAttribute('content', CONFIG.MEMBERSHIP_DESCRIPTION));

      document.querySelectorAll('.nav a, .mobile-nav a').forEach(a =>
        a.classList.toggle('active', a.getAttribute('href') === '/membership'));

      memberEl.innerHTML = `
        <div class="section-header"><h2 class="section-header__title">${Utils.escapeHtml(CONFIG.MEMBERSHIP_HEADING)}</h2><div class="section-header__line"></div></div>
        <p class="membership-desc">${Utils.escapeHtml(CONFIG.MEMBERSHIP_DESCRIPTION)}</p>
        <ul class="membership-perks">
          <li><i class="fa-solid fa-check"></i> Unlock all members-only posts</li>
          <li><i class="fa-solid fa-check"></i> License key delivered to your inbox instantly</li>
          <li><i class="fa-solid fa-check"></i> Works across all browsers &amp; devices</li>
          <li><i class="fa-solid fa-check"></i> No recurring charge — one key, one year</li>
        </ul>
        <button id="membership-btn" class="btn primary" style="margin-top:1.5rem">
          <i class="fa-solid fa-star"></i> ${CONFIG.MEMBERSHIP_CTA} — ${Utils.escapeHtml(CONFIG.MEMBERSHIP_PRICE)}
        </button>
        <p id="membership-status" class="membership-status" style="margin-top:0.75rem; font-size:0.9rem"></p>
      `;

      const memberBtn = document.getElementById('membership-btn');
      const statusEl  = document.getElementById('membership-status');
      const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--color-accent').trim() || '#b53309';

      if (AppState.isMember) {
        memberBtn.disabled = true;
        memberBtn.style.opacity = '0.7';
        ...
        memberBtn.addEventListener('click', function () {
          ...
          const options = {
            ...
            handler: function () {
              ...
              memberBtn.disabled = true;
              statusEl.textContent = 'Check your email in a few minutes for your license key.';
              statusEl.style.color = 'var(--color-success)';
            },
      ...
      } else if (!CONFIG.RAZORPAY_KEY_ID && statusEl) {
        statusEl.textContent = 'Payment not yet configured. Please contact us directly.';
      }
      return;
    }
    ...
    // Hide all non-active views (bookmarks, membership, etc.)
    const bmView2 = document.getElementById('view-bookmarks');
    if (bmView2) bmView2.style.display = 'none';

    // Close search bar only when navigating to a post
...
const UI = {
  showError(title, detail, code) {
    const grid = Utils.qs('#posts-grid');
    if (!grid) return;
    grid.innerHTML = `<div class="empty-state"><i class="fa-solid fa-triangle-exclamation empty-icon" style="color:var(--color-error)"></i><h3>${Utils.escapeHtml(title)}</h3><p>${Utils.escapeHtml(detail)}</p>${code ? `<pre>${Utils.escapeHtml(code)}</pre>` : ''}</div>`;
  },
  ...
  initBackTop() {
    ...
    if (!btn) return;
    const _bto = CONFIG.BACK_TO_TOP_OFFSET || 400;
    window.addEventListener('scroll', () => btn.classList.toggle('show', window.scrollY > _bto), { passive: true });
  ...
  initKeyboardShortcuts() {
    ...
    document.addEventListener('keydown', e => {
      ...
      // G-sequence shortcuts: G then H / B
      if (e.key === 'g' || e.key === 'G') {
        gPressed = true;
        clearTimeout(gTimer);
