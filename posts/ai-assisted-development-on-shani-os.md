...
**OpenCode Zen** — OpenCode's curated gateway ships a smaller rotating free tier (around 5 free models at a time out of 90+ total, capped at 100 requests/day) selectable straight from the model picker. One honest caveat: some free-period models train on collected data, so keep proprietary code on paid or local routes.
...
- **Snapshot before you delegate**: `@home` has no automatic snapshot schedule today, so take a manual Btrfs snapshot yourself before a big autonomous run; revert instantly if the result disappoints.
