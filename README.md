...
### 3. No `posts/index.json` needed

When `GITHUB_USER` and `GITHUB_REPO` are set in CONFIG (the default for production), the blog **auto-discovers** all `.md` files in `posts/` via the GitHub Contents API. Just drop a post file into `posts/`, push, and it appears — no manifest required.

For **local development** without GitHub API access, a `posts/index.json` fallback is still supported as an array of filenames or metadata objects:

```json
["newest-post.md", "older-post.md"]
```

> **Why still needed locally?**
> Browsers cannot list directory contents. The JSON array is a zero-dependency fallback when running `python3 -m http.server` locally.
...
- **No index.json in production** — GitHub Contents API auto-discovers all `.md` files in `posts/`; `index.json` only needed for local dev fallback
- **`/posts/slug` redirect** — visiting `https://blog.shani.dev/posts/why-my-post` (raw GitHub Pages path) is silently rewritten to `/post/why-my-post` and rendered fully styled inside the SPA
