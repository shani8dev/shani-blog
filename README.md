...
A styled table of contents is auto-generated from `h2`/`h3` headings in the post body (minimum 2 headings required). Place the `[toc]` shortcode anywhere in the post to render it at that position:
...
- **Table of contents** — auto-generated from `h2`/`h3` headings via `[toc]` shortcode or auto-detected manual TOC blocks
