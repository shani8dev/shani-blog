...
**How local post discovery works** — three strategies tried in order, first success wins:

| Priority | Strategy | How |
|---|---|---|
| 1 | `posts/manifest.json` | Generated by `node generate-manifest.js` — fastest, recommended |
| 2 | `posts/index.json` | A filename list you maintain manually |
| 3 | Directory listing | `python3 -m http.server` serves a browsable HTML index; the SPA parses its `<a href>` links to find `.md` files automatically — **zero config needed** |

The zero-config path (strategy 3) works out of the box. For correct sort order and instant load, run `generate-manifest.js` once:

```bash
node generate-manifest.js        # generate posts/manifest.json once
node generate-manifest.js --watch  # auto-regenerate on every .md save
python3 -m http.server 8080
```

`generate-manifest.js` produces the exact same `posts/manifest.json` as the GitHub Actions workflow. Once it exists, the SPA uses it directly on both localhost and production.
...
4. Commits and pushes `manifest.json` back to the repo
...
**You need zero secrets.** `GITHUB_TOKEN` is injected automatically by GitHub into every workflow run — you do not create it. It is used only to clone the repo and push `manifest.json` back. It never appears in any output file and never reaches the browser.
...
To deploy: push to `main`. The Action runs, writes `manifest.json`, and GitHub Pages serves the updated site. No manual steps.
...
├── generate-manifest.js    ← Local dev helper: generates posts/manifest.json
├── CNAME                   ← blog.shani.dev
├── robots.txt
├── sitemap.xml
├── README.md
├── posts/
│   ├── index.json          ← Local dev fallback: filename list (maintained manually)
...
> **Note:** `generate-manifest.js` produces the exact same `posts/manifest.json` as the GitHub Actions workflow. Run it once locally before starting a dev server for the best experience — correct sort order and instant post loads without relying on directory listing parsing.
...
- **Batched post fetching** — when manifest is absent, `.md` files are fetched in batches of 12 rather than all at once, keeping the browser connection pool sane at scale
...
- **Localhost-aware** — on `localhost` / `127.0.0.1` / LAN IPs the GitHub Contents API is never called; post discovery falls back to local file strategies only
